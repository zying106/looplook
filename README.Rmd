...
**`refine_loop_anchors_by_chromatin()`** — Applies chromatin-mark evidence to systematically **reclassify** anchors and update loop topologies. Reclassification rules (minimum input H3K4me1 + H3K4me3): P + dual marks → `dual`; P + enhancer marks → `E`; eP/eG + promoter marks → `P`/`G` (revert); E + H3K4me3 only → `P` (unannotated promoter); and more. Supports `recompute_targets = TRUE` to rebuild target gene links with updated anchor types, producing chromatin-aware `target_annotation` and `target_gene_links`.
