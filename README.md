...
against ENCODE active-enhancer criteria: `gold_standard` (H3K4me1+,
H3K27ac+, ATAC+, H3K4me3-, H3K27me3-), `high_confidence`, `supported`,
`weak`, or `uncertain`.

**`refine_loop_anchors_by_chromatin()`** — Applies chromatin-mark
evidence to systematically **reclassify** anchors and update loop
topologies. Reclassification rules (minimum input H3K4me1 + H3K4me3):
P + dual marks → `dual`; P + enhancer marks → `E`; eP/eG + promoter
marks → `P`/`G` (revert); E + H3K4me3 only → `P` (unannotated promoter);
and more. Supports `recompute_targets = TRUE` to rebuild target gene
