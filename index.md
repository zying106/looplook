...
within 10% of max expression). The biotype priority order can be
...
### Module 3B: Chromatin-Guided Reclassification
...
BED files. Each anchor is scored against ENCODE active-enhancer
criteria: `gold_standard` (H3K4me1+, H3K27ac+, ATAC+, H3K4me3-,
H3K27me3-), `high_confidence`, `supported`, `weak`, or `uncertain`.

**[`refine_loop_anchors_by_chromatin()`](https://zying106.github.io/looplook/reference/refine_loop_anchors_by_chromatin.md)**
— Applies chromatin-mark evidence to systematically **reclassify**
anchors and update loop topologies. Reclassification rules (minimum
input H3K4me1 + H3K4me3): P + dual marks → `dual`; P + enhancer marks →
`E`; eP/eG + promoter marks → `P`/`G` (revert); E + H3K4me3 only → `P`
(unannotated promoter); and more. Supports `recompute_targets = TRUE` to
rebuild target gene links with updated anchor types, producing
...
*Figure 4: **Chromatin-Guided Reclassification outputs.** Right: Sankey
flow diagram tracing each anchor’s reclassification path (Before →
After) with colourblind-safe Wong palette. Left: aggregated heatmap
showing the percentage of anchors in each reclassification group
...
##  RSQLite                             3.53.1    2026-05-23 [1] CRAN (R 4.5.2)
