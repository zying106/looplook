...
``` R
           ...
           └──────────┬───────────────┘       └──────────┬────────────────┘
                      ...
                      └────────────────┬─────────────────┘
                                       │
                   Three layers = one integrated framework
           each adds orthogonal evidence, each can correct the others
```
...
loop topologies. H3K4me3-positive anchors default to P or G; true
dual-function elements require either bigWig confirmation
(H3K4me1/H3K4me3 ≥ 3 via `chromatin_bw`) or overlap with a curated
enhancer BED (`enhancer_bed`). Also supports: P + enhancer marks → `E`;
eP/eG + promoter marks → `P`/`G` (revert); E + H3K4me3 only → `P`
...
*Figure 4: **Chromatin-Aware Reclassification outputs.** Right: Sankey
...
##    ggseqlogo                           0.2.2     2025-12-22 [1] CRAN (R 4.5.2)
...
##  P looplook                          * 0.99.14   2026-07-11 [?] Bioconductor
