...
if (has_data) {
  ...
  # Ensure at least 5 genes for heatmap/connectivity modules
  if (length(target_genes) < 5) {
    target_genes <- head(gene_sets[["Target_Genes"]], 12)
  }
...
test_that("plot_summary_go_lollipop returns ggplot objects", {
  ...
  go_df <- data.frame(
    ...
    p.adjust = c(0.05, 0.10, 0.15, 0.08, 0.20, 0.25),
