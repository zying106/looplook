...
test_that("consolidate_chromatin_loops validates parameters", {
  ...
  skip_if(f2 == "", "example data missing")
  expect_error(consolidate_chromatin_loops(files = c(f1, "/bad.bedpe")), "not found")
  expect_error(consolidate_chromatin_loops(files = c(f1, f2), gap = -5), "non-negative")
  expect_error(consolidate_chromatin_loops(files = c(f1, f2), min_consensus = 0), "positive integer")
  expect_error(consolidate_chromatin_loops(files = c(f1, f2), min_raw_score = "a"), "single finite")
  expect_error(consolidate_chromatin_loops(files = c(f1, f2), min_score = "x"), "single finite")
  expect_error(consolidate_chromatin_loops(files = c(f1, f2), roi_mode = "invalid"), "should be one of")
})
...
test_that(".consolidate_intersect handles all-filtered case", {
  ...
  res <- looplook:::.consolidate_intersect(gi_list,
    gap = 1000, n_reps = 2,
...
test_that(".annotation_feature_class classifies all types", {
  ...
  expect_equal(looplook:::.annotation_feature_class(""), "Unknown")
...
test_that(".loop_locus_genes extracts P/G anchor genes", {
  ...
  expect_equal(looplook:::.loop_locus_genes("E", "E", "G1", "G2"), NA_character_)
})

test_that(".ids_to_genes_simple maps anchor IDs to symbols", {
  lookup <- c(A1 = "TP53", A2 = "BRCA1", A3 = "MYC")
  expect_equal(looplook:::.ids_to_genes_simple(c("A1", "A2"), lookup), "TP53;BRCA1")
...
test_that("load_expression_matrix rejects duplicate gene identifiers", {
  tmp <- tempfile(fileext = ".tsv")
  writeLines("Gene\tS1\nTP53\t10\nTP53\t20", tmp)
  expect_error(
    looplook:::load_expression_matrix(tmp),
...
test_that("clean_anchor filters to active genes and reclassifies", {
  ...
  expect_equal(res1$gene, "TP53;MYC")

  # Silent promoter with down=TRUE → eP, positional gene kept
  res2 <- looplook:::clean_anchor("MYC", "P", allow, down = TRUE)
  expect_equal(res2$type, "eP")
  expect_true(!is.na(res2$gene))

  # Silent promoter with down=FALSE → stays P, positional gene kept
...
test_that("profile_target_genes validates seed parameter", {
  ...
  expect_error(
    do.call(profile_target_genes, c(base_args, list(seed = 1.5))),
    "positive integer"
  )
  expect_error(
    do.call(profile_target_genes, c(base_args, list(seed = -1))),
    "positive integer"
  )
  expect_error(
    do.call(profile_target_genes, c(base_args, list(seed = 0))),
    "positive integer"
  )
  expect_error(
    do.call(profile_target_genes, c(base_args, list(seed = c(1, 2)))),
    "positive integer"
  )
  expect_error(
    do.call(profile_target_genes, c(base_args, list(seed = NA_real_))),
    "positive integer"
  )
...
test_that("profile_target_genes group_order parameter", {
  ...
  out <- do.call(
    profile_target_genes,
    c(base_args, list(group_order = c("con", "trt")))
  )
  expect_type(out, "list")
})
...
test_that("extract_target_gene_sets promoter mode uses Regulated_promoter_genes_Filled", {
  ...
  res <- looplook:::extract_target_gene_sets(mock_anno,
    src = "targets",
...
test_that("plot_summary_go_lollipop handles results without CleanLoopType", {
  mock_go <- list(
    data.frame(
      ...
      pvalue = c(0.001, 0.01), p.adjust = c(0.05, 0.10),
      Count = c(10, 5), ONTOLOGY = c("BP", "MF"),
      LoopType = c("EP", "PP"), stringsAsFactors = FALSE
    )
  )
...
test_that(".validate_annotation_params rejects invalid inputs directly", {
  ...
  expect_error(
    looplook:::.validate_annotation_params(-2, 1, 0, 0.5, 0, 1e5),
    "anchor_gap must be"
  )
  expect_error(
    looplook:::.validate_annotation_params(200, 0, 0, 0.5, 0, 1e5),
    "anchor_min_overlap must be"
  )
  expect_error(
    looplook:::.validate_annotation_params(200, 1, 1.5, 0.5, 0, 1e5),
    "anchor_min_frac must be"
  )
  expect_error(
    looplook:::.validate_annotation_params(200, 1, -0.1, 0.5, 0, 1e5),
    "anchor_min_frac must be"
  )
  expect_error(
    looplook:::.validate_annotation_params(200, 1, 0, 0, 0, 1e5),
    "hub_percentile must be"
  )
  expect_error(
    looplook:::.validate_annotation_params(200, 1, 0, 1.5, 0, 1e5),
    "hub_percentile must be"
  )
  expect_error(
    looplook:::.validate_annotation_params(200, 1, 0, 0.5, -1, 1e5),
    "neighbor_hop must be"
  )
  expect_error(
    looplook:::.validate_annotation_params(200, 1, 0, 0.5, 1.5, 1e5),
    "neighbor_hop must be"
  )
  expect_error(
    looplook:::.validate_annotation_params(200, 1, 0, 0.5, 0, 0),
