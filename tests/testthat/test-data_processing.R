...
test_that("consolidate_chromatin_loops validates numeric parameters", {
  ...
  expect_error(consolidate_chromatin_loops(
    files = c(f1, f2),
...
test_that("consolidate_chromatin_loops emits identical mcols schema across all modes", {
  ...
  gi_int <- looplook:::consolidate_chromatin_loops(c(loop1, loop2),
    mode = "intersect",
    gap = 1000, quiet = TRUE, write_output = FALSE
  )
  gi_con <- looplook:::consolidate_chromatin_loops(c(loop1, loop2),
    mode = "consensus",
    gap = 1000, quiet = TRUE, write_output = FALSE
  )
  gi_uni <- looplook:::consolidate_chromatin_loops(c(loop1, loop2),
    mode = "union",
    gap = 1000, quiet = TRUE, write_output = FALSE
  )
  ...
  expect_setequal(
    colnames(S4Vectors::mcols(gi_int)),
