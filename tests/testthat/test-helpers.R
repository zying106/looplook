...
# --- load_expression_matrix: duplicate gene ID error ---
test_that("load_expression_matrix errors on duplicate gene IDs", {
...
test_that("clean_anchor reclassifies eP and eG correctly", {
  # Gene body silent → eG, positional gene kept
  res <- looplook:::clean_anchor("SILENT_GENE", "G", c("ACTIVE"), TRUE)
  expect_equal(res$type, "eG")
  expect_equal(res$gene, "SILENT_GENE")

  # Enhancer stays E (E is not P or G, so no reclassification)
